...
public:
	...
	void GoalCallback(const geometry_msgs::Vector3& msg)
	{
		...
		goal.PositionGoal.x = 0;//msg.x;
		goal.PositionGoal.y = 0;//msg.y;
		goal.PositionGoal.z = 0.8;//msg.z;

		bufferGoal.push_back(goal);
		if (bufferGoal.size() == 1) {//chama o send goal apenas se a fila estivesse vazia, todo o resto é chamado automaticamente pelo cb de completar
			SendGoal();
		}
		/*fabio
		//Send a goal to the server
